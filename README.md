...
### Cache and Deploy Modes

- Replayable quantum results now live in the tracked repo directory `.cache/runs/`.
- Override the cache location with `QAL_CACHE_DIR=/abs/path/to/runs` when you want a private scratch cache.
- Local development can keep `DISABLE_QPU_SUBMISSION` unset and provide `IONQ_API_KEY` to allow fresh QPU jobs.
- Hosted/frontend builds should set `VITE_BACKEND_URL` to the backend origin and may set `VITE_DISABLE_QPU_SUBMISSION=true` to label the UI as cache-only for real-QPU selections.
- Backend deploys can set `DISABLE_QPU_SUBMISSION=true` to forbid new real-hardware jobs while still allowing cached QPU results from `.cache/runs/` to replay. The legacy alias `DISABLE_QPU=true` is still accepted.
...
If a deployment has `DISABLE_QPU_SUBMISSION=true`, explicit `use_qpu` runs fail closed on a cache miss, but a matching record in `.cache/runs/` still replays without any IonQ key present.
