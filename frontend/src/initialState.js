...
function walkSetup(config) {
  ...
  const quantumStep = {
    ...
    description: `Walker starts at node 0. Graph has ${nNodes} nodes. Quantum interference will spread the walker faster.`,
  };
  ...
  const classicalStep = {
    ...
    mean_position: 0,
    std_position: 0,
    description: `Walker starts at node 0. Classical random walk on ${nNodes} nodes.`,
  };
...
function vqeSetup(config) {
  ...
  const quantumStep = {
    ...
    energy: startEnergy,
    params: [],
    description: `Molecule: ${molecule}. Starting VQE with random ansatz parameters. Target ground state: ${exact.toFixed(4)} Ha.`,
  };
...
function hamiltonianSetup(config) {
  ...
  const quantumStep = {
    ...
    description: `Initial state |${"0".repeat(n)}⟩. ${model.charAt(0).toUpperCase() + model.slice(1)} model on ${n} qubits. Time evolution will redistribute probability.`,
  };
