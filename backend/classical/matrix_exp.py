...
def _ising_hamiltonian(n_qubits: int, J: float = 1.0, h: float = 0.5) -> np.ndarray:
    ...
    for i in range(n_qubits - 1):
        ...
        pauli[i] = "Z"
        pauli[i + 1] = "Z"
        H += -J * _pauli_string_matrix("".join(pauli))

    # Transverse field
    for i in range(n_qubits):
        pauli = ["I"] * n_qubits
        pauli[i] = "X"
        H += -h * _pauli_string_matrix("".join(pauli))
...
def _heisenberg_hamiltonian(
    ...
    """Heisenberg model: H = sum [Jx X_i X_{i+1} + Jy Y_i Y_{i+1} + Jz Z_i Z_{i+1}]."""
    dim = 2 ** n_qubits
    H = np.zeros((dim, dim), dtype=complex)
    ...
    for i in range(n_qubits - 1):
        for pauli_char, coupling in [("X", Jx), ("Y", Jy), ("Z", Jz)]:
            ...
            H += coupling * _pauli_string_matrix("".join(pauli))
...
def _build_hamiltonian(
    ...
    **kwargs: float,
    ...
    model = model.lower()
    if model == "ising":
        return _ising_hamiltonian(n_qubits, **kwargs)
    elif model == "heisenberg":
        return _heisenberg_hamiltonian(n_qubits, **kwargs)
    else:
