...
def _drop_params(params: dict[str, Any], *keys: str) -> dict[str, Any]:
    """Return *params* without the named keys."""
...
def _cache_root() -> Path:
    ...
    Uses ``QAL_CACHE_DIR`` if set, else the tracked ``.cache/runs`` directory
    under cwd.
    """
...
def should_cache(backend: str, n_qubits: int, small_n: int = SMALL_N) -> bool:
    """Cache policy. QPU always; simulators at or above SMALL_N."""
...
def _canon(v: Any) -> Any:
    ...
    Critically, an integral float and the equal int must hash identically:
    the UI slider yields ``time=1.0`` while a prior run may have been keyed with
    ``time=1`` — without this they produce different keys and the cache misses.
    ``1.0`` and ``1`` both canonicalize to ``1``; genuine fractions (``0.5``)
    and bools are left untouched.
    """
...
def _legacy_param_variants(module_id: str, params: dict[str, Any]) -> list[dict[str, Any]]:
    """Return backward-compatible param variants for legacy cache keys.

    Hamiltonian-sim cache entries existed before ``interaction_pattern`` and
    ``alpha`` were part of the key. Preserve lookup compatibility so old demo
    caches remain reusable after the UI started surfacing those controls.
    """
    variants: list[dict[str, Any]] = []
    if module_id != "hamiltonian_sim":
        return variants

    pattern = params.get("interaction_pattern", "chain")
    if pattern != "power_law" and "alpha" in params:
        variants.append(_drop_params(params, "alpha"))

    if pattern == "chain" and "interaction_pattern" in params:
        variants.append(_drop_params(params, "interaction_pattern"))
        if "alpha" in params:
            variants.append(_drop_params(params, "interaction_pattern", "alpha"))

    return variants


def candidate_keys(module_id: str, params: dict[str, Any]) -> list[str]:
    """Return current + legacy-compatible cache keys for *params*."""
    seen: set[str] = set()
    out: list[str] = []
    for variant in [dict(params), *_legacy_param_variants(module_id, params)]:
        key = compute_key(module_id, variant)
        if key in seen:
            continue
        seen.add(key)
        out.append(key)
...
def lookup(module_id: str, params: dict[str, Any]) -> tuple[str, list[dict[str, Any]]]:
    """Return the first matching cache key and its valid records.

    The first key is always the current schema. If no cache entry exists under
    the current key, legacy-compatible aliases are checked in order.
    """
    keys = candidate_keys(module_id, params)
    for key in keys:
        records = get(key)
        if records:
            return key, records
...
def aggregate(records: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    ``steps``, ``final_result`` and ``metadata`` come from the newest record
    (last). Returns ``{shots, counts, steps, final_result, metadata,
    n_records}`` or an empty dict if there are no records.
    """
